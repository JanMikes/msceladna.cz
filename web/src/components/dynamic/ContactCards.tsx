...
type CardData = ComponentContactCards['cards'][number];

export function ContactCards({ data, sidebar }: ContactCardsProps) {
  if (!data.cards || data.cards.length === 0) return null;

  const defaultStyle = data.style ?? '1';

  return (
    <div className={sidebar ? 'grid grid-cols-1 gap-6' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'}>
      {data.cards.map((card, i) => {
        const style = card.style ?? defaultStyle;
        return <ContactCard key={i} card={card} style={style} />;
      })}
    </div>
  );
}

function ContactCard({ card, style }: { card: CardData; style: '1' | '2' | '3' }) {
  if (style === '3') {
    return (
...
function ContactMeta({ card }: { card: CardData }) {
  return (
